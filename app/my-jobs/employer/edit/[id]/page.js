...
import { useCreateJobMutation, useGetAJobQuery, useUpdateJobMutation } from "@/redux/slice/job/jobApiSlice";
...
export default function EmployerEditJob({ params }){
    const router = useRouter()
    const [updateJob, { isLoading }] = useUpdateJobMutation()

    const { data, isLoading: isJobDetailsLoading, error } = useGetAJobQuery(params.id)

    // console.log(job)

    const update_employer_job = async (values, actions) => {
        try {
            ...
            router.replace('/my-jobs/employer')
        } catch (err) {
            console.log(err)
    ...
    const { values, errors, touched, handleChange, handleBlur, handleSubmit, setValues } = useFormik({
        ...
        onSubmit: update_employer_job
    })

    // update the initial values with the fetched data
    useEffect(() => {
        if(data){
            setValues({
    ...
    return (
        <DashLayout>
            <div>
                <h2 className="font-bold text-xl text-center">Update a Job</h2>
